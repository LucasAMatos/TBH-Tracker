...
| # | Item | Observável / base | Notas |
|---|------|-------------------|-------|
| I1 | Localizar o save automaticamente | caminho padrão Windows | Suportar override manual de caminho |
| I2 | Descriptografar ES3 (AES-CBC + PBKDF2-SHA1) | arquivo `.es3` | Chave fornecida pelo usuário; nunca no repo |
| I3 | File watcher + fingerprint (mtime+tamanho) | arquivo no disco | Reler só quando muda; evitar descriptografia redundante |
| I4 | Parser save → snapshot tipado | JSON interno | Campos da tabela em TBHPEDIA.md |
| I5 | Estado de conexão | chave / arquivo presentes | Monitorando · sem chave · sem save · erro |
...
- **S1 (P0):** Decodificar `DAPP` → "Dificuldade · Ato · Fase" + onda atual.
- **S2 (P0):** Mostrar estágio máx. concluído.
...
- **H1 (P1):** Nível e XP por herói; herói líder.
- **H2 (P1):** Detectar level-ups (eventos).
- **H3 (P1):** Nº de heróis ativos → normalizar XP/h por herói.
- **H4 (P2):** Composição ativa (nomes via catálogo de heróis).

### Cubo (P2) — `CubeSaveLevelData`
- **C1 (P2):** Nível e XP do Cubo no dashboard.
- **C2 (P2):** Alertas de marcos (ex.: nível 10 = Trade Ship liberado).

### Baús (P1/P2) — `BoxData.BoxQuantity`
- **B1 (P1):** Contagem de baús não abertos.
...
| # | Item | Notas |
|---|------|-------|
| U1 (P0) | Dashboard com cards (ouro, estágio, heróis, cubo, baús, máx. estágio) | MVP |

...
### Pets (P2) — `PetSaveData[]`
- **PE1 (P2) ✅ v1.8.0:** **Pets no Dashboard** — antes o app **não cobria pets**. *Entregue:* catálogo `scripts/gen-pets.cjs` → `src/shared/petData.ts` (8 pets: nome pt-BR, condição de unlock + param1, efeitos `{st,mt,v}` referenciando o catálogo D4); parser lê `PetSaveData[]` (`PetKey,IsUnlock`) → `snapshot.pets`; helper `src/shared/pets.ts` (`petEffectLines`, `petUnlockLabel`, `aggregatePetBonuses`); e widget **"Pets"** no Dashboard (liga/desliga + colapsável, U10) com **desbloqueados/total**, lista por pet (estado + efeitos + como obter) e **bônus ativos somados**. Coberto por testes (`test/pets.test.ts`).

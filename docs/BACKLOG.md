...
- **D5 (P2) ✅ v1.9.0:** **Calculadora de derretimento (Alchemy/Cubo)** — estima **quanto ouro + XP de Cubo** você ganha derretendo o gear. *Entregue:* gerador `scripts/gen-melt.cjs` → `src/shared/meltData.ts` (5.744 itens: `itemSell` + `itemCubeExp` por `ItemKey`); lógica pura `src/shared/melt.ts` (`summarizeMelt`) que soma ouro + XP de Cubo do gear **excluindo equipados e Legendary+** (vendável no Market), com quebra por raridade; parser popula `snapshot.melt` (`MeltSummary`); e widget **"Derretimento (Alchemy)"** no Dashboard (liga/desliga + colapsável, U10) com totais, nº derretível e exclusões. Coberto por testes (`test/melt.test.ts`). *Em aberto:* confirmar se "derreter" usa exatamente `itemSell` ou outra tabela e se o valor escala com o nível do item.

### Pets (P2) — `PetSaveData[]`
- **PE1 (P2) ✅ v1.8.0:** **Pets no Dashboard** — antes o app **não cobria pets**. *Entregue:* catálogo `scripts/gen-pets.cjs` → `src/shared/petData.ts` (8 pets: nome pt-BR, condição de unlock + param1, efeitos `{st,mt,v}` referenciando o catálogo D4); parser lê `PetSaveData[]` (`PetKey,IsUnlock` + detecção tolerante do **pet ativo/equipado**) → `snapshot.pets`; helper `src/shared/pets.ts` (`petEffectLines`, `petUnlockLabel`, `petById`); e widget **"Pets"** no Dashboard (liga/desliga + colapsável, U10) com **desbloqueados/total**, o **bônus do pet ativo** e a lista por pet (estado bloqueado/desbloqueado/ativo + efeitos + como obter). Coberto por testes (`test/pets.test.ts`). *Obs.:* o bônus **não é cumulativo** — só o pet ativo concede o efeito; o campo exato do "pet equipado" no save ainda está sendo confirmado (detecção tolerante por ora).

### 🐞 Bugs conhecidos
- **BUG-PET-ATIVO (P2) — detecção do pet equipado/ativo:** o widget de Pets (PE1) já trata o bônus como **não cumulativo** (só o ativo concede), mas o **campo do save que indica o pet equipado ainda não foi confirmado**. O parser tenta nomes prováveis (`equippedPetKey`/`arrangedPetKey`/`currentPetKey`/`selectedPetKey` no player/common, ou flag `IsEquip`/`Equipped` por entrada em `PetSaveData`); se nenhum casar, **nenhum pet aparece como ativo** (★) e o "Bônus ativo" não é exibido. *A fazer:* inspecionar o JSON bruto de um save real com um pet equipado, identificar o campo correto e fixar a detecção (+ teste). *Workaround atual:* a lista mostra o bônus de cada pet e a regra "só o ativo".
...
- **I9 (P2) ✅ v1.9.0:** **Painel de diagnóstico** — entregue: aba **"Diagnóstico"** (`src/renderer/src/components/Diagnostics.tsx`) reúne num só lugar o **estado da conexão**, **caminho** e **status da chave** do save, **heartbeat** e **última leitura** (relógio + "há Xs"), mais **avisos de catálogo desatualizado** (ItemKeys fora do catálogo via `inventory.unknownCount`, itens sem valor de derretimento, estágio não reconhecido) — sinaliza datamine velho após patch (sinergia com N2). Somente leitura.

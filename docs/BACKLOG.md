...
- **H8 (P2):** **Retratos/ícones dos heróis** — baixar as imagens dos 6 heróis da TBH Wiki (`taskbarhero.wiki/pt/heroes`, ver `FONTES.md`) e exibi-las nos cards de ativos (dashboard, H6) e na aba **Heróis** (H5). Mesmo padrão dos ícones de runa (v4.0): assets versionados em `src/`, mapeados por `heroKey`.
- **H9 (P2):** **Análise detalhada do herói** (drill-down dentro da aba Heróis), modelada na página da wiki (ex.: `taskbarhero.wiki/pt/heroes/ranger`): papel/arma principal+secundária, condição de desbloqueio, **stats base** (Dano, Vel. de ataque, Crít., Dano crít., PV, Armadura, Vel. de movimento, Vel. de conjuração, Red. recarga) com o **ranking do atributo entre os 6 heróis**, e a **árvore de habilidades por tier** (Tiers 1–8; passivas/ativas com nível máx. e custo em pontos por tier). *Observável:* nível do herói (`HeroLevel` → pontos de habilidade disponíveis, 1/nível) sobreposto ao **catálogo** (datamine de stats + árvore). Requer catálogo `src/shared/heroes.ts` (stats + skill tree dos 6).

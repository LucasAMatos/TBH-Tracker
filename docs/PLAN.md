...
- **Fase 2 — Farm analytics:** ✅ *núcleo entregue (v20.0).* ouro/h e xp/h por estágio (**F2**, `stageFarm.ts`), histórico persistente (**F3**), recomendação de melhor estágio ouro/xp/combo (**F4**, `rankStages`) e **Aba de Farm** (**U2**), sobre a fundação **F0 (v18.0)** (catálogo `stageData.ts`). *Pendentes:* **F1** (detecção de fim de corrida) segue **bloqueado** pelo save; **F5** (projeção de estágios não medidos) e o refino da recomendação com medições reais ficam para depois.

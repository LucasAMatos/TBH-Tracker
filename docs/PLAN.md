...
- **Fase 5 — Extras:** suporte Proton/Linux, ~~marcos do Cubo~~ (✅ v0.3.0 / C2), ~~níveis/mapa de runas~~ (✅ v0.4.0 / R1), calibração de gasto de runas, ~~exportação de dados~~ (✅ v1.3.0 / E1 — sessão JSON + farm CSV), ~~calculadora de derretimento/Alchemy~~ (✅ v1.9.0 / D5 — ouro + XP de Cubo recuperável, `meltData.ts`).
...
> trava regressões ao regenerar catálogos e ajustar trackers. **Painel de diagnóstico** (✅ v1.9.0 /
> I9 — aba "Diagnóstico": estado/caminho/chave/heartbeat + avisos de catálogo desatualizado).
> Próximo de infra no `BACKLOG.md › Aplicativo / engenharia` (N2 detecção de patch).
>

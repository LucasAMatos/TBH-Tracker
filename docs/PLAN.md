...
- **Fase 2 — Farm analytics:** ✅ *núcleo entregue (v0.20.0).* ouro/h e xp/h por estágio (**F2**, `stageFarm.ts`), histórico persistente (**F3**), recomendação de melhor estágio ouro/xp/combo (**F4**, `rankStages`) e **Aba de Farm** (**U2**), sobre a fundação **F0 (v0.18.0)** (catálogo `stageData.ts`). **F1** (eficiência de farm: clears estimados, clears/h e tempo por clear via kills ÷ inimigos por clear) ✅ v1.2.0 — só o tempo por **corrida individual** segue inviável pelo save. *Pendente:* o refino da recomendação com medições reais fica para depois.
